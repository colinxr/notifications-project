...
export declare type AnyRef<M> = ComputedRef<M | null> | Ref<UnwrapRef<M> | null>;
export declare type DiffDefinition = undefined | string | string[] | Record<string, any> | false;
...
export declare type HandledEvents = 'created' | 'patched' | 'updated' | 'removed';
export declare type HandleEventsFunction<M extends BaseModel = BaseModel> = (item: any, ctx: {
    model: ModelStatic<M>;
    models: any;
}) => any;
export declare type HandleEvents<M extends BaseModel = BaseModel> = {
    [event in HandledEvents]: HandleEventsFunction<M>;
};
export interface DefineStoreOptionsWithDefaults<Id extends string, S extends StateTree, G, A, DefaultS extends StateTree, DefaultG, DefaultA> extends DefineStoreOptionsBase<S, Store<Id, S, G, A>> {
    state?: () => S;
