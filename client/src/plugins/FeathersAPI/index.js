							...
							async authenticate(authData) {
								try {
									if (typeof authData == 'undefined')
										authData = {
											strategy: 'jwt',
											accessToken: this.feathersClient.settings.storage.storage['feathers-jwt'] || this.accessToken
										};
									const response = await feathersClient.authenticate(authData, additionalParams);
...
const serviceFindMixin = (options) => {
	...
	const nameToUse = name ? name : typeof service == 'string' ? service.replace(/-./g, (x) => x[1].toUpperCase()) : 'items';
	if (typeof service !== 'function' && typeof stores[service] == 'undefined') throw new Error('Invalid Service Name: ' + service);
...
const serviceGetMixin = (options) => {
	...
	const afterFetch = options.afterFetch;
	const debounceTime = options.debounce || 500;
	const serviceStore = stores[typeof service == 'function' ? service(this) : service]();
	const nameToUse = name
		? name
		: typeof service == 'string'
		? serviceStore.Model.name.charAt(0).toLowerCase() + serviceStore.Model.name.slice(1)
		: 'item';
	const capitalizedName = nameToUse.charAt(0).toUpperCase() + nameToUse.slice(1);
	const FETCH = 'fetch' + capitalizedName;
	const DO_FETCH = '_doFetch' + capitalizedName;
...
export { FeathersAPI as default, getServiceStore, mapServiceState, mapServiceActions, serviceFindMixin, serviceGetMixin };
